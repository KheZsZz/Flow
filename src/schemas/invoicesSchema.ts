...
export const invoiceSchema = z.object({
    ...
    nature_transaction: z.string().min(1, 'A natureza da operação é obrigatória').max(255),
    weight_brute: z.string().min(1, 'O peso bruto é obrigatório').max(255),
    quantity_volumes: z.string().min(1, 'A quantidade de volumes é obrigatória').max(255),
